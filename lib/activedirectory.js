...
defaultAttributes = originalDefaultAttributes = {
  user: [ 
    ...
    'comment', 'description', 'primaryGroupID'
  ],
  group: [
    'dn', 'cn', 'description'
  ]
...
/**
 * Get the Group objectSid from the User objectSid and primaryGroupID.
 * 
 * @private
 * @param {String} userSid value of the User objectSid.
 * @param {String} primaryGroupID value of the Group RID.
 */
var Long = require("long");

function getGroupSid(userSid, primaryGroupID) {
	/*S-1-5-21-1414102034-2213856796-643364766-4680*/
	console.log('userSid', userSid);
	var groupSid = 'S-';
	var version = userSid.charCodeAt(0);
	var countAuth = userSid.charCodeAt(1);
	groupSid += version + '-';
	var authority = 0;
	for(let i = 2; i <= 7; i++) {
        authority |= (userSid.charCodeAt(i) << (8 * (5 - (i - 2))));
    }
	groupSid += authority + '-';
	var offset = 8;
	var size = 4;
	var subAuthority = new Long(0xFFFFFFFF, 0x7FFFFFFF);
	subAuthority = 0;
	for (let j = 0; j<countAuth-1; j++){
		subAuthority = 0;
		for(let k = 0; k <size; k++) {
			subAuthority |= userSid.charCodeAt(offset + k) << (8 * k);
			console.log(userSid.charCodeAt(offset + k));
			
		}
		offset += size;
		groupSid += subAuthority + '-';
	}
	console.log('groupSid', groupSid);
}

function decodeSID(sid, primaryGroupID) {
	...
	/*S-1-5-21-1414102034-2213856796-643364766-4680*/
	    ...
	    const authority_bytes = sid.slice(2, 8)

	    var authority = Long.fromBytesBE(authority_bytes,true).shiftRight(2 * 8)
	  ...
	  //iterate all the sub-auths
	    ...
	    for(var j = 0; j < countSubAuths-1; j++) {

	        subAuthority_bytes = sid.slice(offset, size);
	        subAuthority = Long.fromBytesLE(subAuthority_bytes,true);
	    ...
	    strSid += "-" + primaryGroupID;
	    console.log('strSid', strSid);
	    return strSid;
	}
...
function getGroupMembershipForDN(opts, dn, stack, callback) {
  ...
  let filter = '(member='+parseDistinguishedName(dn)+')';
  let ll = false;
//  console.log(opts.primaryGroupID);
  if (opts.primaryGroupID) {
//	  filter = '(|' + filter + '(objectSid=*-' + opts.primaryGroupID + '))';
//	  filter = '(objectSid=S-1-5-21-1414102034-2213856796-643364766-4680)';
//	  var objectSid = getGroupSid(opts.userSid, opts.primaryGroupID);
	  var objectSid = decodeSID(opts.userSid, opts.primaryGroupID);
	  filter = '(objectSid=' + objectSid + ')';
//	  filter = '(cn=IT Support)';
//	  filter = '(objectSid=' + opts.userSid.slice(0, -5) +  opts.primaryGroupID + ')';
	  opts.primaryGroupID = undefined;
	  ll = true;
  }
  opts = _.defaults(_.omit(opts || {}, 'filter', 'scope', 'attributes'), {
    filter: filter,
    scope: 'sub',
    attributes: joinAttributes((opts || {}).attributes || defaultAttributes.group, [ 'groupType', 'objectSid' ])
  });
    ...
    async.forEach(results, function(item, asyncCallback) {
      if (isGroupResult(item)) {
        ...
        if (includeGroupMembershipFor(opts, 'user')) {
          opts.primaryGroupID = user.primaryGroupID;
          opts.userSid = user.objectSid;
          console.log('objectSID', user.objectSid);
